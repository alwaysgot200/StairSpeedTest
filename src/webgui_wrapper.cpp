...
extern std::string speedtest_mode, export_sort_method, export_color_style,
    custom_group, override_conf_port;
...
void json_write_node(rapidjson::Writer<rapidjson::StringBuffer> &writer,
                     nodeInfo &node) {
  ...
  writer.Key("address");
  writer.String(
      std::string(node.server + ":" + std::to_string(node.port)).data());
  writer.Key("info");
  writer.String(
      std::string(
          (inbound.country.size() ? inbound.country : std::string("N/A")) +
          " " + (inbound.city.size() ? inbound.city : std::string("N/A")) +
          ", " +
          (inbound.organization.size() ? inbound.organization
                                       : std::string("N/A")))
          .data());
  writer.EndObject();
  writer.Key("outbound");
  ...
  writer.String(
      std::string(
          (outbound.country.size() ? outbound.country : std::string("N/A")) +
          " " + (outbound.city.size() ? outbound.city : std::string("N/A")) +
          ", " +
          (outbound.organization.size() ? outbound.organization
                                        : std::string("N/A")))
          .data());
  writer.EndObject();
  writer.EndObject();
...
void ssrspeed_webserver_routine(const std::string &listen_address,
  ...
  append_response("GET", "/status", "text/plain",
                  [](RESPONSE_CALLBACK_ARGS) -> std::string {
                    return start_flag ? "running" : "stopped";
                  });

  /*append_response("GET", "/", "REDIRECT", [](RESPONSE_CALLBACK_ARGS) ->
  std::string
  {
      return "http://web1.ospf.in/";
  });*/

  append_response("GET", "/favicon.ico", "x-icon",
                  [](RESPONSE_CALLBACK_ARGS) -> std::string {
                    return fileGet("tools/gui/favicon.ico", true);
                  });

  append_response("GET", "/getversion", "text/plain",
                  [](RESPONSE_CALLBACK_ARGS) -> std::string {
